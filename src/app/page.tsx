...
export default function Home() {
  ...
  // 加载最近的吐槽
  const loadRecentRoasts = async (isLoadMore = false) => {
    try {
      if (isLoadMore) {
        setLoadingMore(true);
      }
  ...
  const saveAsImage = async () => {
    ...
    try {
      ...
      // 改进html2canvas配置，更好地处理图像
      const canvas = await html2canvas(resultCardRef.current, {
        scale: 2, // 高分辨率
        backgroundColor: "#ffffff",
        logging: false,
        useCORS: true, // 启用CORS以支持跨域图片
        allowTaint: true, // 允许跨域图片（可能会污染画布）
        ...
        onclone: (clonedDoc) => {
          ...
          return new Promise(resolve => setTimeout(resolve, 300)); // 给DOM时间渲染
        }
      });
    ...
    <div className="min-h-screen bg-[#f5f5f7] flex flex-col">
      ...
      <main className="flex-1 py-8">
        <div className="max-w-4xl mx-auto px-4">
          ...
          {/* 结果展示区域 - 苹果风格卡片 */}
          {result && (
            <div id="result-section" className="bg-white rounded-2xl shadow-sm overflow-hidden animate-fade-in mb-8" ref={resultCardRef}>
              {/* 博主信息区域 */}
              ...
              <div className="p-6">
                ...
                <div className="mt-6 pt-4 border-t border-gray-100 flex flex-wrap justify-between items-center">
                  <p className="text-sm text-gray-500 flex items-center mb-3 md:mb-0">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 mr-1">
